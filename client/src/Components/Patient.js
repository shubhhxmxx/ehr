...
class Patient extends Component {
    ...
    componentDidMount(){ 
        this.loadPatient();    
        console.log("called loadPatient");
    }
    ...
    async loadFiles(){
        ...
        console.log('called load file',files);
        if(files[0])  this.setState({files:files});

    }
    async loadPatient (){
        let res = await this.contract.methods.getPatientInfo().call({from :this.accounts[0]});
        console.log("patient load:",res);
        if(res[0]=="") alert("user not registered");
    ...
    async uploadFile(event)
    {
        ...
        client.add(this.state.buffer).then((res)=>{
            console.log(res);
            console.log(res.path);
            console.log(res[1]);
            console.log(res[2]);
           this.updateFileHash(this.state.file.name,this.state.file.type,res.path)
        })
    }
    getFile(event)
    {
