...
class Owner extends Component {
  ...
  async addUserByAdhaar(event){
    ...
    try{
      ...
      console.log(adhaar_number);
      console.log(adhaar_blockchain_id);
      console.log(pincode);

      if(user_type == 'patient'){
        result = await this.contract.methods.addPatientAdhaarInfo(adhaar_blockchain_id, user_name, dob, pincode, adhaar_number,contact,gender).send({ from: this.accounts[0] });
        console.log("success");

      }
      else{
        result = await this.contract.methods.addDoctorAdhaarInfo(adhaar_blockchain_id, user_name, dob, pincode, adhaar_number,contact,gender).send({ from: this.accounts[0] });
          ...
          <h4 style={{ alogn: "centre" }}>Add User by Adhaar</h4>
            <div>
              <form onSubmit={this.addUserByAdhaar}>
                ...
                <div className="label mt-2">Contact:</div>
                <input type="text" id="contact" placeholder="contact"></input>
                <br></br>

                <div className="label mt-2">Gender:</div>
